...
the child playing as a leader. If it is not obvious that a 5-7 aged child
would be able to take this role, it is actually uncertain that he would sustain
his engagement during a long session (40-60 minutes). Through different case
studies, this article aims to explore experimental designs and scenaries that
help children to gain confidence and motivation over repetitives such long
sessions. Then it introduces a measurement of how children was satisfyed
...
demonstrations. This measure could  be a first step to automaticly get
...
2D-space and can be seen as vector with 140 coordinates
...
guidance of his occupational therapist to make the robot starting from simple
vertical stroke for all letters (round 0 on Figure~\ref{learning_6_demos}). In
the thrid study~\ref{auto} we chose to use middle point between vertical stroke
...
used to generate better eigenspace with PCA. Recurrent neural networks (RNN)
...
The second question focus on the leanring algorithm. In
~\cite{Hood}, we were projecting children's demonstrations in PCA's eigenspace in order to 
compute the middle between that point and previous state projected as well. Then, we just had to
...
ideas: In the first study~\ref{diego} we generated by PCA an eigenspace from a
small set of allographs we arbitrary drawn. Each time the child was providing a
demonstration, we added that demonstration to the small set and re-built the
eigenspace by PCA. Then we used the same way to generate the new state
from the middle point between demonstration and last state in the eigenspace.
From our perspective, this eigenspace was more adapted to the child's
progression, and the sequence of tries performed by the robot looked smoother.
But using metric in eigenspace can make the learning algorithm too slow in some
cases, because consecutives projected demonstrations are sometimes too
far from each other in eigenspace while they looks close in cartesian space.
...
\begin{figure}
    ...
    \caption{\small Thomas's demonstrations for the number ``6'' (top row) and
...
blue lines Thomas's performance. The round IDs correspond to the demonstrations
...
P_0=0$.
To mesure if the fact that the child was rewarding the robot when it was
...
\begin{table}
    ...
    \end{tabular}
    \caption{\small results of evaluation. demo : number of demonstrations provided by
...
the interaction design and the learning algorithm that succeeded to maintain
...
understand the activity and feel that the robot actually learn from his
...
understanding of the activity and about how much he was satisfyed by the
learning process (both the robot's ability to learn and the child's own
ability to teach). We believe that this information could be taken in account by
...
the robot while its level decreases), or if the child did not understood the
activity (if he never uses the evaluation buttons and spend a lot of time to provide
a resonse). (2) We can simply reinforce the learnt allograph when the robot
